...
public class WordManager {
    ...
    public void Write(List<TaskDto> taskDtos) {
        for (TaskDto taskDto : taskDtos) {
            ...
            if (!(taskDto.getSolutionDto().getKY() == 0.0 || taskDto.getSolutionDto().getShY() == 0 )) {
            ...
            if (!(taskDto.getProblemDto().getTarget().getTargetsFrontDuOP() == 0 || taskDto.getProblemDto().getTarget().getTargetsDepthOP() == 0.0)) {
                builder2.append(String.format("Фц (оп) = %s\tГл (оп) = %d\t", formatTextDivision((int) Math.round(taskDto.getProblemDto().getTarget().getTargetsFrontDuOP())), (int) Math.round(taskDto.getProblemDto().getTarget().getTargetsDepth())));
            }
    ...
    private String formatTextDivision(int value) {
        if (Math.abs(value) > 99){
            if (value > 0) {
                ...
                return String.format("-%d-%d", Math.abs(value / 100), Math.abs(value % 100));
            }
        } else {
            return value > 0
                    ? String.format("+0-%s",
